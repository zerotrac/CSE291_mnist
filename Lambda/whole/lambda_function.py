...
def lambda_handler(event, context):
    for record in event["Records"]:
        ...
        images_path = '/tmp/{}'.format(const.IMAGES_NAME)
        s3_client.download_file(const.TRAINING_BUCKET, const.IMAGES_NAME, images_path)
        labels_path = '/tmp/{}'.format(const.LABELS_NAME)
        s3_client.download_file(const.TRAINING_BUCKET, const.LABELS_NAME, labels_path)

        # 第一步：读 mnist 训练集的图片，将图片二值化、压缩后进行存储，节省空间
        train_images = read_mnist_image(images_path, const.TRAIN_SIZE)

        # 第二步：读 mnist 训练集的标注
        train_labels = read_mnist_label(labels_path, const.TRAIN_SIZE)
        ...
        # 第五步：重新打开待 OCR 的图片，框出所有数字（根据第三步存储的数据）并标出（根据第四步的结果）
        collect_result(download_path, upload_path, boxes, predictions)

        s3_client.upload_file(upload_path, const.RESULT_BUCKET, key)
