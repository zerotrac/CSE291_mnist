...
def main(params, action):
    ...
    with open(filename, "rb") as fin:
        ...
        # serialize images
        imageBytes = pickle.dumps(images)
        length = len(imageBytes)

        # send length meta data to remote memory server
        struct.pack_into('@I', trans.buf, 0, length)
        print(length)
        print("Finish append length")
        trans.write(4, 0, 0)

        # append images data to local buffer
        trans.buf[0:length] = imageBytes
        print("Finish append data")

        # send images data to remote memory server
        begin = 0
        remoteIndex = 4
        blockSize = 1000000
